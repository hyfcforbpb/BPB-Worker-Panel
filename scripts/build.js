...
async function buildWorker() {
    ...
    if (mangleMode) {
        ...
        // 关键优化：保护 WebSocket 判定相关的关键字，防止逻辑失效
        const obfuscationResult = obfs.obfuscate(minifiedCode.code, {
            compact: true,
            controlFlowFlattening: false, // 禁用控制流平坦化，提高执行速度
            deadCodeInjection: false,     // 禁用死代码注入，防止 CPU 负载过高导致握手失败
            stringArray: true,
            stringArrayThreshold: 1,
            stringArrayEncoding: [],      // 禁用 RC4/Base64 加密，确保 Header 匹配极速且准确
            reservedStrings: [            // 强制保留 WebSocket 关键字符不被加密
                'Upgrade', 
                'websocket', 
                'globalConfig', 
                'pathName',
            ...
            transformObjectKeys: false,   // 防止破坏 request.headers 的访问逻辑
            renameGlobals: false,         // 确保核心变量不被破坏
            target: "browser"
        });
        console.log(`${success} Worker obfuscated successfuly! (Optimized for WebSocket)`);
